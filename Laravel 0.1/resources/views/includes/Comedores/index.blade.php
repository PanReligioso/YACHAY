...
@section('title', 'YACHAY - Comedores Universitarios')
...
<section class="section">
    <div class="container">
        <form method="GET" action="{{ url('/comedores') }}" style="margin-bottom: var(--spacing-2xl); padding: var(--spacing-xl);
            ...
            <div class="grid grid-4" style="gap: var(--spacing-xl); align-items: flex-end;">
                ...
                <div>
                    ...
                    <select name="precio" id="precio" style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--text-tertiary); border-radius: var(--radius-md);">
                        ...
                        <option value="economico" {{ $precio_filter == 'economico' ? 'selected' : '' }}>Económico (< S/ 5)</option>
                        <option value="moderado" {{ $precio_filter == 'moderado' ? 'selected' : '' }}>Moderado (S/ 5 - 10)</option>
                        <option value="premium" {{ $precio_filter == 'premium' ? 'selected' : '' }}>Premium (> S/ 10)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="comida" id="comida" style="width: 100%; padding: var(--spacing-sm); border: 1px solid var(--text-tertiary); border-radius: var(--radius-md);">
                        ...
                        <?php foreach($tipos_comida as $tipo): ?>
                            <option value="{{ strtolower($tipo) }}" {{ strtolower($comida_filter) == strtolower($tipo) ? 'selected' : '' }}>
                                {{ $tipo }}
                            </option>
                        <?php endforeach; ?>
                    </select>
            ...
            <div style="text-align: right; margin-top: var(--spacing-lg);">
                ...
                <a href="{{ url('/comedores') }}" class="btn btn-outline" style="margin-left: var(--spacing-sm);">
        ...
        <h2 style="font-size: var(--text-2xl); color: var(--text-primary); margin-bottom: var(--spacing-xl);">
            Resultados ({{ $total_comedores }} Comedores encontrados)
        </h2>
            ...
            <div class="grid grid-3" style="gap: var(--spacing-2xl);">
                <?php foreach($comedores_filtrados as $comedor): ?>
                    <div class="card" style="box-shadow: var(--shadow-md); transition: transform 0.2s; border: 1px solid var(--bg-secondary);">
                        <img src="{{ $comedor['foto'] }}" alt="{{ $comedor['nombre'] }}"
                        ...
                        <div style="padding: var(--spacing-lg);">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: var(--spacing-sm);">
                                ...
                                <div style="display: flex; align-items: center; gap: 4px; font-size: var(--text-sm); font-weight: bold; color: var(--accent-yellow);">
                                    {!! display_stars($comedor['valoracion_promedio']) !!}
                                    <span style="color: var(--text-secondary); margin-left: var(--spacing-xs);">({{ number_format($comedor['valoracion_promedio'], 1) }})</span>
                                </div>
                            </div>
                            ...
                            <ul style="list-style: none; padding: 0; margin-bottom: var(--spacing-lg); font-size: var(--text-sm);">
                                <li style="margin-bottom: var(--spacing-xs); color: var(--text-secondary);">
                                    ...
                                    Rango Precio: **S/ {{ number_format($comedor['precio_menu_min'], 2) }} - S/ {{ number_format($comedor['precio_menu_max'], 2) }}**
                                </li>
                                <li style="margin-bottom: var(--spacing-xs); color: var(--text-secondary);">
                                    <i class="fas fa-clock" style="color: var(--accent-orange);"></i>
                                    Horario: {{ $comedor['horario_apertura'] }} - {{ $comedor['horario_cierre'] }} ({{ $comedor['dias_atencion'] }})
                                </li>
                                <li style="margin-bottom: var(--spacing-xs); color: var(--text-secondary);">
                                    <i class="fas fa-concierge-bell" style="color: var(--secondary-600);"></i>
                                    Tipo: {{ $comedor['tipo_comida'] }}
                                </li>
                                <li style="margin-bottom: var(--spacing-xs); color: var(--text-secondary);">
                                    <i class="fas fa-walking" style="color: var(--primary-600);"></i>
                                    Distancia Aprox: **{{ get_distance_approx($comedor['id']) }} caminando**
                                </li>
                            </ul>

                            <div style="display: flex; gap: var(--spacing-md); margin-top: var(--spacing-lg);">
                                <a href="{{ url('/comedores/' . $comedor['id']) }}" class="btn btn-primary" style="flex: 1;">
        ...
        <?php else: ?>
            <div style="text-align: center; padding: var(--spacing-3xl); background: var(--bg-secondary); border-radius: var(--radius-xl);">
                ...
                <a href="{{ url('/comedores') }}" class="btn btn-outline" style="margin-top: var(--spacing-xl);">
...
{{--    La función 'initMap' ahora funciona porque está en el objeto 'window' --}}
<script async defer
